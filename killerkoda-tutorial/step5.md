...
In this step, you'll scan the Docker image of the vulnerable Flask app using Grype to detect OS and package vulnerabilities within the container.

## Why container image scanning?

- **Catches base image CVEs** that code scans miss
- **Detects OS-level packages** (apk/apt/rpm) and language packages
- **Complements SAST and dependency scans** for broader coverage
...
```bash
cd vulnerable-app
docker build -t vulnshop:latest .
```{{exec}}

## Scan the image with Grype

```bash
grype vulnshop:latest -o json > ../grype-results.json
echo "Grype results saved to grype-results.json"
```{{exec}}
...
if [ -f "../grype-results.json" ]; then
  jq -r '.matches[] | "- Vulnerability: \(.vulnerability.id) | Severity: \(.vulnerability.severity) | Artifact: \(.artifact.name)"' ../grype-results.json | head -n 20 || true
fi
```{{exec}}

## Optional: Fail on high/critical

```bash
cat > grype-security-gate.sh << 'EOF'
#!/bin/bash
set -e
FILE="grype-results.json"
[ -f "$FILE" ] || { echo "No grype-results.json"; exit 1; }
CRIT=$(jq '[.matches[] | select(.vulnerability.severity=="Critical")] | length' "$FILE")
